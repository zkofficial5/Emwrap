...
const Reports = () => {
  ...
  useEffect(() => {
    ...
    Promise.all(
      months.map((m) =>
        api
          .get(`/workspaces/${workspace.id}/reports/summary?month=${m}`)
          .then((res) => ({
            month: m.slice(5), // just MM
            total: res.data.total || 0,
          })),
      ),
    )
      .then((results) => setMonthlyReports(results))
  ...
  const fetchSummary = async () => {
    ...
    try {
      const res = await api.get(
        `/ai/report-summary?workspace_id=${workspace.id}&month=${selectedMonth}`,
      );
  ...
  const handleDownloadWithAi = async () => {
    try {
      const res = await api.get(
        `/workspaces/${workspace.id}/reports/export?ai=true&month=${selectedMonth}`,
        { responseType: "blob" },
      );
  ...
  const handleDownloadCsv = async () => {
    try {
      const res = await api.get(
        `/workspaces/${workspace.id}/reports/export?month=${selectedMonth}`,
        { responseType: "blob" },
      );
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const a = document.createElement("a");
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-md border border-border bg-secondary px-3 py-2 text-sm text-muted-foreground transition-colors hover:text-foreground"
          >
      ...
      >
        ...
        <AnimatePresence>
          {summaryOpen && summary && (
            ...
            >
              <div className="mx-5 mb-5 rounded-lg border border-primary/20 bg-primary/5 p-4">
